...
class SchoolBuses extends React.Component {
  ...
  buildSearchParams = () => {
    ...
    if (
      this.state.search.selectedCitiesIds.length > 0 &&
      this.state.search.selectedCitiesIds.length !== _.size(this.props.cities)
    ) {
  ...
  updateUIState = (state, callback) => {
    this.setState({ ui: { ...this.state.ui, ...state } }, () => {
      store.dispatch({
        type: Action.UPDATE_BUSES_UI,
        schoolBuses: this.state.ui,
      });
      if (callback) {
        callback();
      }
    ...
    Api.sendEmail(email)
      .then((response) => {
        if (
          ...
          //get valid response from server
          this.openEmailSendConfirm();
        } else {
          //invalid response from server or mailSent == false
          this.openEmailSendFail(email, response);
        }
      })
      .catch((err) => {
        //when server pod down or get response from server timeout
    ...
    return (
      <div id="school-buses-list">
          ...
          <Row>
            <Col md={10}>
              <Row>
                <ButtonToolbar id="school-buses-filters">
                  ...
                  />
                  <MultiDropdown
                    id="selectedCitiesIds"
                    placeholder="Cities"
                    items={cities}
                    selectedIds={this.state.search.selectedCitiesIds}
                    updateState={this.updateSearchState}
                    showMaxItems={2}
                  />
                  <MultiDropdown
                    id="selectedSchoolDistrictsIds"
                    placeholder="School Districts"
                    items={schoolDistricts}
                    selectedIds={this.state.search.selectedSchoolDistrictsIds}
                    fieldName="shortName"
                    updateState={this.updateSearchState}
                    showMaxItems={2}
                  />
              ...
              <Row>
                <ButtonToolbar id="school-buses-inspections">
                  <DropdownControl
                    id="nextInspection"
                    title={this.state.search.nextInspection}
                    updateState={this.updateSearchState}
                    items={[
                      ALL,
                      BEFORE_TODAY,
                      BEFORE_END_OF_MONTH,
                      BEFORE_END_OF_QUARTER,
                      TODAY,
                      WITHIN_30_DAYS,
                      THIS_MONTH,
                      NEXT_MONTH,
            ...
            <Col md={2}>
              <Row id="school-buses-faves">
                <Favourites
                  id="school-buses-faves-dropdown"
                  type="schoolBus"
        ...
        {this.state.showEmailDialog && (
          <SchoolBusesEmailDialog
            schoolBuses={this.props.schoolBuses}
        ...
        {this.state.showEmailSendFailDialog && (
          <EmailSendFailDialog
            show={this.state.showEmailSendFailDialog}
            onClose={this.closeEmailSendFail}
            onRetry={this.retryEmailSend}
            errorMessage={this.state.emailSendErrorMessage}
          />
        )}
      </div>
    );
